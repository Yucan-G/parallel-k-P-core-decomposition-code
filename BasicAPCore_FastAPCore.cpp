/*********** Code for BasicAPCore&FastAPCore ***********/
...
vector<matrix> A;
vector<triplet> edgeTriplets;
vector<int> path;
vector<int> core;
int path_length;
double HomBCore_runtime, BasicAPCore_runtime, FastAPCore_runtime;
...
int load_HIN(char* filename)
{
	...
	for (int i = 0; i < g.n; i++) { g.v.push_back(tmp_v); edgeTriplets.push_back(tmp_t); core.push_back(0);}
	...
	while (fscanf(infp, "%d %d %d\n", &u, &v, &type) != EOF) {
		...
		g.v[v].push_back(e);
		Triplet t = {u, v, 1};
		edgeTriplets[type - 1].push_back(t);
	}
	fclose(infp);
	for (int i = 0; i < g.t; i++)
	{
		setFromTriplets(A[i], edgeTriplets[i]);
	}
	return 0;
}
...
#pragma omp parallel for num_threads(NUM_THREADS)
	for (int i = 0; i < g.n; i++)
	{
		...
		for (int j = 0; j < g.v[i].size(); j++)
			if (g.v[i][j].type == ini_type)
			{
				flg = 1; break;
			}
		if (flg) //v is a vertice with the target type
		{
			vector<int> x;
			x.push_back(i);
			for (int l = 0; l < path_length; l++)
			{
				...
				{
					int cur_v = x[k];
					for (int m = 0; m < g.v[cur_v].size(); m++)
						if (g.v[cur_v][m].type == cur_t) y.insert(g.v[cur_v][m].v);
				}
...
void BasicAPCore()
{
	...
	timeval time_over;
	timeval time1, time2;
	cout << "Ready to run BasicAPCore..." << endl;
	gettimeofday(&time_start,NULL);
	gettimeofday(&time1,NULL);
	...
	cout << "HomBCore running time / BasicAPCore running time = " << HomBCore_runtime / BasicAPCore_runtime << endl << endl;
}
...
void FastAPCore()
{
	...
	cout << "HomBCore running time / FastAPCore running time = " << HomBCore_runtime / FastAPCore_runtime << endl;
